...
use crate::{
    ...
    cache::CacheStore,
    diff::{find_parsed_diff_file, DiffLineKind, ParsedDiffFile, ParsedDiffHunk},
...
pub const REVIEW_PARTNER_GENERATOR_VERSION: &str = "review-partner-v26";
pub const REVIEW_PARTNER_CONTEXT_VERSION: &str = "review-partner-context-v9";

const REVIEW_PARTNER_CACHE_KEY_PREFIX: &str = "review-partner-v26";
...
fn build_review_partner_prompt(bundle: &ReviewPartnerContextBundle) -> String {
    ...
    [
        ...
        "Usage rows are generated by Remiss from language-server references or labeled fallback matches. Leave usage lists out of the JSON.",
...
fn usage_groups_for_target(
    ...
    layer
        ...
        .chain(layer.removed_symbols.iter().map(|symbol| (symbol, true)))
        .filter_map(|symbol| {
            let (symbol, removed_symbol) = symbol;
            let usages = symbol
                .references
                .iter()
                .filter(|location| {
                ...
                .collect::<Vec<_>>();
            if !should_show_usage_group(symbol, removed_symbol, usages.len()) {
                return None;
            }
            Some(ReviewPartnerUsageGroup::new(
                symbol.symbol.clone(),
                usage_group_summary(symbol, removed_symbol, usages.len()),
                usages,
            ))
        })
        .take(MAX_SECTION_ITEMS)
...
fn should_show_usage_group(
    symbol: &ReviewPartnerCollectedSymbol,
    removed_symbol: bool,
    usage_count: usize,
) -> bool {
    if usage_count == 0 {
    ...
    if is_lsp_usage_strategy(&symbol.search_strategy) || removed_symbol {
        return true;
    }
    high_signal_fallback_usage_symbol(&symbol.symbol)
}

fn usage_group_summary(
    symbol: &ReviewPartnerCollectedSymbol,
    removed_symbol: bool,
    usage_count: usize,
) -> String {
    let plural = if usage_count == 1 { "" } else { "s" };
    if is_lsp_usage_strategy(&symbol.search_strategy) {
        format!("{usage_count} reference{plural} resolved by the language server.")
    } else if removed_symbol {
        format!(
            "{usage_count} remaining match{plural} surfaced by {}.",
            symbol.search_strategy
        )
    } else {
        format!(
            "{usage_count} syntax match{plural} surfaced by {}.",
            symbol.search_strategy
        )
    }
}

fn is_lsp_usage_strategy(strategy: &str) -> bool {
    strategy == "lsp references"
}

fn high_signal_fallback_usage_symbol(symbol: &str) -> bool {
    if symbol.chars().any(char::is_whitespace) || low_signal_usage_symbol(symbol) {
        return false;
    }
    let has_namespace = symbol.contains("::");
    let has_snake_word = symbol.contains('_') && symbol.len() >= 8;
    let starts_like_type = symbol
        .chars()
        .next()
        .map(|ch| ch.is_ascii_uppercase())
        .unwrap_or(false)
        && symbol.len() >= 5;
    has_namespace || has_snake_word || starts_like_type
}

fn low_signal_usage_symbol(symbol: &str) -> bool {
    let lowercase = symbol.to_ascii_lowercase();
    if matches!(
        lowercase.as_str(),
        "default"
            | "github"
    ...
    let has_lowercase = symbol.chars().any(|ch| ch.is_ascii_lowercase());
    let alphanumeric_count = symbol
        .chars()
        .filter(|ch| ch.is_ascii_alphanumeric())
        .count();
    if !has_lowercase && alphanumeric_count >= 3 {
        return true;
    }
    !symbol.contains('_') && symbol.chars().all(|ch| ch.is_ascii_lowercase()) && symbol.len() < 8
}
